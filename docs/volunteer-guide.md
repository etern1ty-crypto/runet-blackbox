...
По умолчанию используется системный резолвер ОС. Если Windows показывает DNS `ECONNREFUSED` или нужно сравнение, используй `--compare-dns`: primary остаётся системным, а comparison resolver записывается отдельной проверкой.
...
`--dns` и `--dns-server` одинаковы и используют явный DNS как primary. `--compare-dns` безопаснее для отчёта о реальной сети, потому что TCP/TLS/HTTP остаются привязаны к системному DNS. Это диагностическое сравнение, не обход.
